...
case "contact-info":
  ...
  // IT WILL HELP IN CHOOSING THE PREVIEW AS WELL AS HEKPS IN DOWNLOADING THE RESUME AND ALSO SAVE THE RESUME AS A IMAGE
const uploadResumeImages = async () => {
  try {
    setIsLoading(true);
    
    // Wait a bit for the component to render if needed
    ...
    if (!thumbnailElement) {
      console.error('thumbnailRef.current is null');
      console.error('Available refs:', Object.keys(thumbnailRef));
      
      // Try to find the element by other means
      const fallbackElement = document.querySelector('[data-thumbnail]') || 
                             document.querySelector('.thumbnail') ||
                             document.getElementById('thumbnail');
      
      if (fallbackElement) {
        console.log('Using fallback element:', fallbackElement);
        // Use fallbackElement instead
      } else {
        throw new Error("Thumbnail element not found. Please ensure the resume preview is loaded.");
      }
    }
  ...
  // THIS FUNCTION WILL HELP IN UPDATING THE RESUME DETAILS
  ...
  const downloadPDF = async () => {
    ...
    // TYPE OF HOW IT WILL LOOK ON DOWNLOAD
    ...
    try {
      await html2pdf()
        ...
        .from(element)
        .save(); // WE SAVE THE RESUME HERE AND THEN WE DOWNLOADED IT
